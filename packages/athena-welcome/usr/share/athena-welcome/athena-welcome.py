...
Do you want to install it?")
    ...
    def mirror_update(self):
        ...
        subprocess.run(["pkexec", "/usr/bin/mirroars", "-n", "21", "-m", "15", "-p", "w", "-t", "-r", "blackarch", "/etc/pacman.d/blackarch-mirrorlist", "-w"], shell=False)
        GLib.idle_add(self.cc.set_markup, "<span foreground='orange'><b><i>Updating Chaotic AUR mirrorlist</i></b> \nThis may take some time, please wait...</span>")  # noqa
        subprocess.run(["pkexec", "/usr/bin/mirroars", "-n", "21", "-m", "15", "-p", "w", "-t", "-r", "chaotic-aur", "/etc/pacman.d/chaotic-mirrorlist", "-w"], shell=False)
